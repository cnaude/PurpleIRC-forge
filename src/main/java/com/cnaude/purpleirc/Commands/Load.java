    ...
    public void dispatch(CommandSender sender, String[] args) {
        if (args.length >= 2) {
            ...
            if (plugin.ircBots.containsKey(bot)) {
                sender.sendMessage(EnumChatFormatting.RED + "Sorry that bot is already loaded. Try to unload it first.");
                return;
            }
            File file = new File(plugin.botsFolder, bot);
            if (file.exists()) {
                sender.sendMessage(EnumChatFormatting.WHITE + "Loading " + bot + "...");
                ...
                sender.sendMessage(EnumChatFormatting.RED + "No such bot file: " + EnumChatFormatting.WHITE + bot);
            }
        } else {
